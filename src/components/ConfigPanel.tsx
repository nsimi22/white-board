...
export default function ConfigPanel() {
  ...
  const canSave = validated && projectKey;
  const domainHint = domain && !domain.includes('.')
    ? `${domain}.atlassian.net`
    : cleanDomain(domain);

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-[#0d1526] border border-[#1e2f57] rounded-2xl shadow-2xl text-slate-100">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e2f57]">
          <div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[#162244] border border-[#1e2f57] rounded-lg px-3 py-2.5 text-sm text-slate-100
                         placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/40"
            />
            {domain && domain !== domainHint && (
              <p className="text-xs text-slate-400 mt-1">Will use: <span className="text-indigo-400">{domainHint}</span></p>
            )}
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="ml-2 text-xs text-indigo-400 hover:text-indigo-300 font-normal"
              >
                Get a token ↗
              </a>
            </label>
        ...
        <div className="px-6 pb-5 flex justify-end gap-3">
          <button
            onClick={() => setIsConfigOpen(false)}
            className="px-4 py-2 text-sm text-slate-400 hover:text-slate-200 rounded-lg hover:bg-[#1e2f57] transition-colors"
          >
            Cancel
          </button>
          <button
